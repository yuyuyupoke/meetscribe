...
BINARY="$BUILD_DIR/arm64-apple-macosx/$CONFIG/MeetScribe"
if [[ ! -f "$BINARY" ]]; then
...
cp "$BINARY" "$APP_DIR/Contents/MacOS/MeetScribe"
...
echo "==> Signing"
ENTITLEMENTS="$PROJECT_DIR/Resources/MeetScribe.entitlements"
# 自己署名証明書 (MeetScribe Dev) があればそのSHA1で署名。
# find-identity ではなく find-certificate で探す (自己署名は codesigning policy で
# valid として出てこないが、SHA1 指定なら codesign は使える)。
SIGN_ID="-"
CERT_SHA1=$(security find-certificate -c "MeetScribe Dev" -Z login.keychain 2>/dev/null \
...
if [[ -n "$CERT_SHA1" ]]; then
    ...
    if codesign --force --sign "$CERT_SHA1" "$TEST_FILE" >/dev/null 2>&1; then
        SIGN_ID="$CERT_SHA1"
        echo "    using self-signed 'MeetScribe Dev' ($CERT_SHA1)"
    else
