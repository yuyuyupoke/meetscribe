...
- **通話アプリと安全に併用可能** — マイクは生キャプチャ（OSデフォルト入力をそのまま使用）。Discord / Zoom 等と同時に使ってもマイクを奪わない。スピーカー利用で相手の声が `[自分]` に二重記録される場合のみ、エコーキャンセル（Voice Processing）をオプトインできる（`defaults write com.meetscribe.app echoCancellationEnabled -bool true`。ON 中は通話アプリとの併用不可）。
