...
> これに整形・対訳・Copilot のトークン料金が加わる。実運用では無音送信の抑制が
> 効くため、実測はこれより低くなる。
...
> xAI Streaming STT は音声 1 時間あたり $0.20。マイクとシステム音声の 2 ストリームで
> 最大約 $0.40/時。これに整形・対訳・Copilot のトークン料金が加わり、実測では
> **1 時間の会議でおおむね $0.5〜1** に収まる（発話量による）。アプリのヘッダーに
> 会議ごとの実費が表示される。
